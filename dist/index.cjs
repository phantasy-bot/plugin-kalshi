  ...
  getTools() {
    return [
      ...
      },
      {
        ...
        description: "List open Kalshi 15m BTC/ETH (and other *15M) crypto up/down markets. No public 5m series.",
        parameters: {
          type: "object",
          properties: {
            series: {
        ...
        handler: async (params) => {
          const service = await this.ensureService();
          const seriesList = (str(params.series) || "KXBTC15M,KXETH15M").split(",").map((s) => s.trim()).filter(Boolean);
          const markets = [];
